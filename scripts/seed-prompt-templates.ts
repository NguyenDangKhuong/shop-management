/**
 * Seed script: drop + re-insert prompt templates from PROMPT.md
 * Run: npx ts-node --compiler-options '{"module":"commonjs"}' -r tsconfig-paths/register scripts/seed-prompt-templates.ts
 */
import fs from 'fs'
...
const TEMPLATES = [
    ...
    {
        title: 'Prompt 4 — Mirror selfie che mặt, không thoại',
        group: 'che mặt + silent',
        order: 4,
        subPrompt: SUB_PROMPT,
    ...
    {
        title: 'Prompt 5 — Tay chạm show vải che mặt, không thoại',
        group: 'che mặt + silent',
        order: 5,
        subPrompt: SUB_PROMPT,
    ...
    {
        title: 'Prompt 6 — Nghiêng nhẹ show dáng che mặt, không thoại',
        group: 'che mặt + silent',
        order: 6,
        subPrompt: SUB_PROMPT,
...
async function seed() {
    ...
    const PromptTemplateModel = (await import('../src/models/PromptTemplate')).default

    // Drop old data and re-seed
    await PromptTemplateModel.deleteMany({})
    console.log('🗑️  Cleared old templates')

    await PromptTemplateModel.insertMany(TEMPLATES)
