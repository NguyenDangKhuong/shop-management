...
export async function GET() {
    try {
        ...
        // Auto-refresh avatars for users missing one (best-effort, non-blocking)
        const usersWithoutAvatar = users.filter((u) => !u.avatarUrl)
        if (usersWithoutAvatar.length > 0) {
            // Fire-and-forget: update missing avatars in background
            Promise.all(
