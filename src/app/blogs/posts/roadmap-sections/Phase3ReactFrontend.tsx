      ...
      <Suspense fallback={<CartSkeleton />}>
        <CartStatus />
                ...
                <TopicModal title="Server Components & Server Actions" emoji="🖥️" color="#0ea5e9" summary="RSC = zero JS bundle, Server Actions = RPC-style mutations — kiến trúc mới của React/Next.js" concept="Server Components render trên server, gửi HTML thuần cho client — 0 JS bundle, truy cập DB/API trực tiếp. Client Components ('use client') chạy trên browser cho interactivity (hooks, events). Server Actions ('use server') là functions chạy trên server được gọi từ client (form submissions, mutations) — không cần tạo API route riêng.">
                    ...
                    <CodeBlock title="rsc-pattern.tsx">{`// ===== SERVER COMPONENT (Mặc định) =====
// File: app/products/page.tsx
import { db } from '@/lib/database'
import ProductList from './ProductList' // Component tương tác phía client

export default async function ProductsPage() {
    // 1. Fetch dữ liệu trực tiếp từ database ở Server Component
    const products = await db.product.findMany({
    ...
    return (
        <div className="p-6">
            <h1 className="text-2xl font-bold mb-4 font-mono text-cyan-400">Danh sách Sản phẩm ({products.length})</h1>
            {/* 2. Truyền dữ liệu xuống Client Component để xử lý tương tác */}
            <ProductList initialProducts={products} />
        </div>
    )
}

// ===== CLIENT COMPONENT (Tương tác UI & gọi Action) =====
...
export default function ProductList({ initialProducts }) {
    ...
    const handleDelete = async (id: string) => {
        ...
        // Optimistic UI update (xóa ngay trên client trước để tạo trải nghiệm mượt mà)
        ...
        startTransition(async () => {
            ...
            if (result?.error) {
                alert(result.error)
                setProducts(oldProducts) // Rollback nếu server báo lỗi
            }
        })
    }
    ...
    return (
        <div className="space-y-4">
            {/* Form thêm nhanh dùng React 19 useActionState */}
            ...
            <div className="space-y-2 mt-4">
                {products.map(p => (
                    <div key={p.id} className="flex justify-between items-center p-3 border border-white/5 rounded-lg bg-slate-900/40">
                        {editingId === p.id ? (
                            <div className="flex gap-2 w-full">
                                ...
                                <button onClick={() => handleSaveEdit(p.id)} disabled={isPending} className="px-3 py-1 bg-green-600 rounded">Lưu</button>
                                <button onClick={() => setEditingId(null)} className="px-3 py-1 bg-slate-600 rounded">Hủy</button>
                            </div>
                        ) : (
                            <>
                                <span className="font-mono text-slate-300">{p.name} - <strong className="text-cyan-400">{p.price}đ</strong></span>
                                <div className="flex gap-2">
                                    <button onClick={() => {
                                        setEditingId(p.id)
                                        setEditName(p.name)
                                        setEditPrice(p.price)
                                    }} className="px-3 py-1 bg-blue-600/30 border border-blue-500/30 text-blue-400 rounded hover:bg-blue-600/50 transition">Sửa</button>
                                    <button onClick={() => handleDelete(p.id)} disabled={isPending} className="px-3 py-1 bg-red-600/30 border border-red-500/30 text-red-400 rounded hover:bg-red-600/50 transition">Xóa</button>
                                </div>
                            </>
                        )}
                    </div>
                    ...
                    <CodeBlock title="server-actions.tsx">{`// ===== SERVER ACTIONS (Thêm/Xóa/Sửa chạy hoàn toàn trên Server) =====
...
// 1. Action Xóa sản phẩm
export async function deleteProduct(id: string) {
    try {
        await db.product.delete({ where: { id } })
        revalidatePath('/products') // Làm mới cache Next.js Router
        return { success: true }
    } catch (error) {
        return { error: 'Lỗi server: Không thể xóa sản phẩm.' }
    }
}

// 2. Action Cập nhật sản phẩm
export async function updateProduct(id: string, data: { name: string; price: number }) {
    if (!data.name || data.price <= 0) {
        return { error: 'Tên và giá sản phẩm không hợp lệ.' }
    }
    try {
        ...
        return { error: 'Lỗi server: Không thể cập nhật sản phẩm.' }
    }
}

// 3. Action Thêm sản phẩm (React 19 Form Action compatible)
export async function createProduct(prevState: any, formData: FormData) {
    ...
    try {
        ...
        return { error: 'Lỗi server: Không thể tạo sản phẩm mới.' }
    }
}
...
export default function QuickAddForm({ onAdd }) {
    // useActionState giúp quản lý pending state và state trả về từ Server Action
    const [state, formAction, isPending] = useActionState(createProduct, null)

    useEffect(() => {
        if (state?.success && state.product) {
            onAdd(state.product) // Đẩy sản phẩm mới lên danh sách UI
        }
    }, [state, onAdd])

    return (
        <form action={formAction} className="flex gap-2 p-3 bg-slate-800/60 border border-white/5 rounded-lg items-center">
            <input name="name" placeholder="Tên sản phẩm..." required className="p-1 rounded text-black bg-white" />
            <input name="price" type="number" placeholder="Giá..." required className="p-1 rounded text-black bg-white w-24" />
