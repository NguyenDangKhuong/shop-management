// Cloudflare R2 Video Upload Utility (via Presigned URL — direct upload to R2)
...
// Cloudflare R2 Video Delete Utility
export const deleteVideoFromR2 = async (fileName: string, bucketName?: string): Promise<{ success: boolean; message?: string }> => {
    try {
        const url = `/api/r2-video?fileName=${encodeURIComponent(fileName)}`
